...
def _extract_text(content) -> str:
    ...
    Handles both plain string and list-of-block formats.
    Joins with "" — each token block already carries its own spacing.
    """
...
def _strip_function_calls(content) -> str:
    """Normalize content to string, then strip leaked <function=...> blocks."""
...
if user_input:
        ...
        def ai_only_stream():
            ...
            while True:
                ...
                if (
                    ...
                    # ── FIX: extract raw token text without stripping per-chunk ──
                    # Groq streams content as individual token blocks. Each token
                    # already carries its own spacing. We extract the raw text here
                    # and defer _strip_function_calls to after joining all chunks.
    ...
    if not stream_error["value"]:
        if needs_translation:
            ...
            # ── FIX: join all chunks first, THEN strip function calls once ──
            # Stripping per-chunk was consuming whitespace between tokens.
