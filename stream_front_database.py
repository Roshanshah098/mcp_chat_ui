...
from lang_data_backend import botbhai, retrieve_all_threads
...
def get_thread_label(thread_id):
    """Build a label from the first user message in the thread."""
    ...
    for msg in raw_msgs:
        if isinstance(msg, HumanMessage):
            ...
            return f"{short}"  # no timestamp since we don't store it in DB

    return f"New Chat · {datetime.now().strftime('%b %d, %I:%M %p')}"
...
if "chat_labels" not in st.session_state:
    st.session_state["chat_labels"] = {}
    # ← on refresh, rebuild labels from DB for all known threads
    for tid in st.session_state["chat_threads"]:
        st.session_state["chat_labels"][tid] = get_thread_label(tid)
...
if user_input:
    ...
    CONFIG = {
        ...
        },  # for easier retrieval in checkpointer,
        "run_name": "chat_run",  # helpful for tracing
    }
